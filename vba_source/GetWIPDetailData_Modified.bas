...
' Override defaulting variables (5.72p — must be procedure-scoped, not inside loop)
Dim defaultRev As Double
Dim defaultCost As Double
Dim calcBonus As Double
Dim opsPctComp As Double
...
If Datars.EOF <> True Then
    ...
    For i = 1 To T
        ...
        ' =====================================================================
        ' Override columns: default to Vista values when no explicit override.
        ' Michael's LCGWIPCreateBatch defaulted these — without them, all
        ' downstream formulas (% Complete, Earned Rev, Proj Profit) show $0.
        ' =====================================================================
        defaultRev = 0
        defaultCost = 0
        calcBonus = 0
        ...
        If sh.CodeName <> "Sheet12" Then ' Ops
            ...
            ' --- Ops Revenue override or default ---
            If Datars.Fields("OpsRevPlugged").Value = "Y" Then
                ...
                defaultRev = Datars.Fields("OpsRev").Value
            Else
                ' Default to Vista projected revenue (Michael's LCGWIPCreateBatch logic)
                ' Fallback chain: Override → ProjContract → BilledAmt
                If Datars.Fields("ProjContract").Value = 0 _
                   And (Datars.Fields("OrigContractAmt").Value + Datars.Fields("COContractAmt").Value) = 0 Then
                    defaultRev = Datars.Fields("BilledAmt").Value
                Else
                    defaultRev = Datars.Fields("ProjContract").Value
                End If
                sh.Range("SummaryData").Cells(r, NumDict(sh.CodeName)("COLOvrRevProj")) = defaultRev
            End If

            ' --- Ops Cost override or default ---
            If Datars.Fields("OpsCostPlugged").Value = "Y" Then
                ...
                defaultCost = Datars.Fields("OpsCost").Value
            Else
                ' Default to Vista projected cost with floor at actual cost
                If Datars.Fields("ProjCost").Value = 0 Then
                    defaultCost = Datars.Fields("ActualCost").Value
                Else
                    defaultCost = Datars.Fields("ProjCost").Value
                End If
                ' Cost floor rule: override cost can't be less than actual cost
                If defaultCost < Datars.Fields("ActualCost").Value Then
                    defaultCost = Datars.Fields("ActualCost").Value
                End If
                sh.Range("SummaryData").Cells(r, NumDict(sh.CodeName)("COLOvrCostProj")) = defaultCost
            End If

            ' --- Bonus Profit ---
            ' Write 0 to break the circular template formula (W→Z→Y→W).
            ' Michael calculated bonus at batch creation time in LCGWIPCreateBatch
            ' using the final override values. We can't do that here because
            ' MergeOverridesOntoSheet hasn't run yet — the override values in I/M
            ' are Vista defaults, not the final LylesWIP values. Writing a calculated
            ' bonus here would use the wrong inputs (e.g., 51.1158 bonus was $43K
            ' with Vista ProjCost but should be ~$0 with Nicole's override).
            ' The correct bonus comes from LylesWIP via MergeOverridesOntoSheet.
            If Datars.Fields("BonusProfitPlugged").Value = "Y" Then
                ...
                calcBonus = Datars.Fields("BonusProfit").Value
            Else
                calcBonus = 0
                sh.Range("SummaryData").Cells(r, NumDict(sh.CodeName)("COLJTDBonusProfit")) = 0
            End If
            sh.Range("SummaryData").Cells(r, NumDict(sh.CodeName)("COLZOPsBonus")) = calcBonus
        ...
        Else ' GAAP
            ...
            ' --- GAAP Revenue override or default ---
            ...
            Else
                ' Default to Vista projected revenue (same chain as Ops)
                If Datars.Fields("ProjContract").Value = 0 _
                   And (Datars.Fields("OrigContractAmt").Value + Datars.Fields("COContractAmt").Value) = 0 Then
                    sh.Range("SummaryData").Cells(r, NumDict(sh.CodeName)("COLOvrRevProj")) = Datars.Fields("BilledAmt").Value
                Else
                    sh.Range("SummaryData").Cells(r, NumDict(sh.CodeName)("COLOvrRevProj")) = Datars.Fields("ProjContract").Value
                End If
            End If

            ' --- GAAP Cost override or default ---
...
' C3d: Add 6-month trend tooltip comments on override Rev/Cost columns.
' Queries LylesWIP for months (wipMonth-6) through (wipMonth-1).
LylesWIPData.MergeTrendCommentsOntoSheet sh, CInt(Co), CDate(Mo)

' C3e: Add 6-month Vista projected revenue/cost trend tooltips on PM columns.
' Queries bJCOR/bJCOP for months (wipMonth-6) through (wipMonth-1).
VistaData.MergeVistaTrendCommentsOntoSheet sh, CInt(Co), CDate(Mo)
